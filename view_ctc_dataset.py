...
img_dir = Path("~/projects/dissertation/data/training/Fluo-N2DH-SIM+/02").expanduser()

img_files = sorted(img_dir.glob("*.tif"))
img_stack = np.stack([tifffile.imread(f) for f in img_files])

seg_dir = Path("~/projects/dissertation/data/training/Fluo-N2DH-SIM+/02_GT/TRA").expanduser()
